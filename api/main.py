...
# Infrastructure helpers extracted to api/helpers.py
from api.helpers import (
    _evict_if_full, _DICT_MAX_SIZE, _DICT_EVICT_BATCH,
    _tracked_write as _tracked_write_impl,
    _safe_key_hash, _validate_webhook_url, _dns_cache, _DNS_CACHE_TTL,
    _extract_client_ip, _is_whitelisted_ip, _WHITELISTED_IP_PREFIXES,
    _track_key_activity, _key_activity, _key_activity_lock,
    _KEY_ACTIVITY_MAX_KEYS, _KEY_ACTIVITY_WINDOW_S,
    _KEY_ACTIVITY_IP_THRESHOLD, _KEY_ACTIVITY_RPM_MULTIPLIER,
    _check_public_rate_limit as _check_public_rate_limit_impl,
    _PUBLIC_RL_LIMIT, _PUBLIC_RL_WINDOW,
)
...
def _tracked_write(d: dict, key: str, value, dict_name: str):
    ...
    _tracked_write_impl(d, key, value, dict_name, write_times=_dict_write_times)
...
def _run_periodic_cleanup():
    ...
    try:
        _managed = {
            ...
            "_feed_subscribers": _feed_subscribers,
            "_zapier_hooks": _zapier_hooks,
            "_make_hooks": _make_hooks,
            "_async_jobs": _async_jobs,
            "_agent_identities": _agent_identities,
            "_mined_patterns": _mined_patterns,
            "_compiled_policies": _compiled_policies,
            "_twin_jobs": _twin_jobs,
            "_clone_history_by_tenant": _clone_history_by_tenant,
            "_passports": _passports,
            "_forensics": _forensics,
            "_blackbox": _blackbox,
            "_lifecycle_policies": _lifecycle_policies,
            "_personas": _personas,
            "_immunity_jobs": _immunity_jobs,
            "_immunity_certs": _immunity_certs,
            "_redteam_jobs": _redteam_jobs,
            "_lab_jobs": _lab_jobs,
            "_healing_counters": _healing_counters,
            "_fidelity_certs": _fidelity_certs,
            "_compression_locks": _compression_locks,
        }
    except NameError:
...
# _safe_key_hash, _validate_webhook_url, _dns_cache, _DNS_CACHE_TTL — imported from api.helpers
...
def _check_public_rate_limit(request: Request, endpoint_name: str) -> dict:
    """IP-based rate limit for public endpoints. Delegates to api.helpers."""
    return _check_public_rate_limit_impl(request, endpoint_name, UPSTASH_REDIS_URL or "", UPSTASH_REDIS_TOKEN or "")
...
# Anomaly detection (_is_whitelisted_ip, _track_key_activity, _key_activity, etc.) — imported from api.helpers
import collections as _collections  # still needed by other code
...
# ---- Memory Vaccination endpoints — moved to api/routers/vaccines.py ----
...
# ---- SLA endpoints — moved to api/routers/sla_feeds.py ----
...
# ---- Verified Memory Registry — moved to api/routers/registry.py ----
...
# ---- Feed endpoints — moved to api/routers/sla_feeds.py ----
...
# ---- #31 SLA Monitoring — handlers moved to api/routers/sla_feeds.py ----
...
# Detection layers extracted to api/detection.py (pure functions, no shared state)
...
from api.routers import registry as _registry_router  # noqa: E402
from api.routers import sla_feeds as _sla_feeds_router  # noqa: E402
...
app.include_router(_registry_router.router)
app.include_router(_sla_feeds_router.router)
