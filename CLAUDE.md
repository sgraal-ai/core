...
- **`dashboard/`** — Decision Readiness Dashboard (Next.js, deployed to Vercel at app.sgraal.com). Pages: `/` fleet overview with OmegaMeter gauges and agent cards, `/agent/[id]` detail with component breakdown/repair plan/at-risk warnings/compliance violations, `/verify` Z3 verification status per profile. Mock data with 5 agents. Deploy: `cd dashboard && vercel --prod`.
