...
- **`sdk/python/`** — `sgraal` PyPI package (`pip install sgraal`). `SgraalClient` with `preflight()` and `signup()` methods. `@guard()` decorator wraps functions with automatic preflight checks, configurable `block_on` level. `GeminiGuard` and `OpenAIGuard` wrap google-generativeai and openai SDKs with automatic preflight — BLOCK stops LLM call, WARN adds context to prompt, USE_MEMORY passes through. `StepTracker` context manager for memory access tracking. Publish: `cd sdk/python && python -m build && twine upload dist/*`.
