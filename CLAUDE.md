...
- **`scoring_engine/constants.py`** — DecisionAction enum (USE_MEMORY, WARN, ASK_USER, BLOCK), thermodynamic constants (Landauer), default decision thresholds, phase constant κ_MEM.
- **Edge mode** — `sgraal.edge` zero-dependency SDK for offline/embedded scoring. No Redis, no Supabase, no HTTP.
- **W3C Verifiable Credentials** — `POST /v1/certify` issues SgraalProof2026 VCs for compliance evidence.
- **Plugin system** — Registry-only with per-tenant isolation.
- **NIST AI RMF compliance** — `GET /v1/compliance/nist-ai-rmf`.
- **Per-type BLOCK thresholds** — Opt-in via `per_type_thresholds=true` in preflight request.
- **Parallel scoring** — Opt-in via `parallel_scoring=true` in preflight request.
- **Governance Score** — Composite 0-100 field on every preflight response.
- **Decision trail** — `decision_trail` field showing every override in the preflight pipeline.
- **Scoring warnings** — `scoring_warnings` field capturing module errors during preflight.
- **6 formal proofs** — Lyapunov stability, Banach contraction, Z3 SMT verification, CTL model checking, sheaf cohomology consistency, ZK commitment.
- **11 benchmark rounds** — Rounds 1-9 (original) + Rounds 10-11 (extended adversarial).
...
# Run tests (2,533+ tests across 25 test files)
...
# Run edge mode (no network required)
python3 -c "from sgraal.edge import edge_preflight; print(edge_preflight([{'id':'m1','content':'test','type':'tool_state','timestamp_age_days':3,'source_trust':0.7,'source_conflict':0.3}]))"
```
...
- pytest: 2,533+ passing
- Corpus total: 614 baseline (Rounds 1-8) + 216 adversarial + 120 Round 9 + 240 Rounds 10-11 = 1,190+ adversarial cases
...
### Test files (33 files, 2,533+ tests):
