...
- **`scoring_engine/`** — Core Ω_MEM computation engine (pure Python, no dependencies). `omega_mem.py` contains the weighted scoring formula using 9 risk components (freshness, drift, provenance, propagation, recall, encode, interference, recovery, r_belief), scaled by action-type and domain criticality multipliers. R_belief (weight 0.05) measures model belief divergence — low belief (<0.2) suggests external memory storage, weak belief (0.2–0.4) suggests user verification.
